...
def point_in_time_fundamentals(history: pd.DataFrame, as_of: pd.Timestamp) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    visible = history.loc[filed < cutoff].copy()
    if visible.empty:
        return {}, visible
    ...
    metadata = {"ticker", "period_end", "filed_at", "fiscal_year", "fiscal_period", "form", "accession"}
    ...
    visible = pd.DataFrame(consolidated).sort_values("period_end").tail(8)
    ...
    for metric in ("revenue", "eps_diluted", "operating_cash_flow", "free_cash_flow", "total_assets", "shares_outstanding"):
        if metric in visible:
            ...
            output[f"{metric}_yoy"] = _growth(values, 4)
    ...
    for metric in ("revenue", "net_income", "free_cash_flow"):
        ...
        output[f"ttm_{metric}"] = float(values.tail(4).sum(min_count=4))
    eps_values = pd.to_numeric(visible.get("eps_diluted", pd.Series(dtype=float)), errors="coerce")
    output["ttm_eps"] = float(eps_values.tail(4).sum(min_count=4))
    for name, value in margins.items():
        ...
        output[f"{name}_change_yoy"] = series.iloc[-1] - series.iloc[-5] if len(series) >= 5 else np.nan
