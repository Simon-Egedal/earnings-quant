...
def calculate_metrics(frame: pd.DataFrame, return_column: str = "strategy_return") -> dict[str, float]:
    ...
    events_per_year = len(returns) / years if pd.notna(years) and years > 0 else np.nan
    ...
    return {
        ...
        "sharpe_ratio": float(returns.mean() / returns.std() * np.sqrt(events_per_year))
        if returns.std() > 0 and pd.notna(events_per_year) else np.nan,
        "sortino_ratio": float(returns.mean() / downside.std() * np.sqrt(events_per_year))
        if downside.std() > 0 and pd.notna(events_per_year) else np.nan,
